...
## STEP 7: CRITICAL — Verify .ACK vs .ACKED File Extension

FinCEN's official SDTM documentation (received via secure message Jan 2026) confirms the acknowledgement file extension is **`.ACK`**, NOT **`.ACKED`**.
...
**Search the entire codebase for `.ACKED` and verify/fix:**

```bash
cd api
grep -rn "ACKED\|\.acked\|\.ACK" --include="*.py" .
```

**Files most likely affected:**
- `api/app/services/fincen/response_processor.py` — parser function name and file lookup
- `api/app/services/filing_lifecycle.py` — `poll_sdtm_responses()` file pattern matching
- `api/app/routes/admin.py` — artifact download endpoint (artifact type name)
- `api/app/services/fincen/fbarx_builder.py` (deprecated but check references)
- `api/app/services/fincen/rerx_builder.py`
- `docs/KilledSharks-2.md` — documentation references

**What to fix:**
- Any code looking for `filename + ".ACKED"` must change to `filename + ".ACK"`
- Any payload_snapshot key `"acked"` can stay as-is (internal naming) but the **SFTP filename pattern** must use `.ACK`
- Update `parse_acked_xml` references if the function name implies `.ACKED` file extension
- Update KilledSharks-2.md references

**This is a silent failure bug** — if the code polls for `.ACKED` but FinCEN writes `.ACK`, your poller will never find acknowledgement files. Filings will appear stuck in "submitted" forever and eventually escalate to `needs_review` after 5 days.
